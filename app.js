...
function sessionWords() {
  ...
  if (currentScope === "due") {
    return due.slice(0, Math.max(1, settings.duePerSession));
  }
  if (currentScope === "fresh") {
    return fresh.slice(0, Math.max(1, settings.freshOnlyPerSession));
  }
  const randomFresh = fresh.slice(0, Math.max(1, settings.freshPerSession));
...
function renderLibrary() {
  return `
    ...
    <div id="library-results" style="margin-top:12px">${libraryResultsHtml()}</div>
  `;
}
...
function libraryResultsHtml() {
  ...
  return `${rows || '<div class="empty">没有匹配的词</div>'}${filtered.length > shown.length ? `<div class="note" style="margin-top:10px">匹配 ${filtered.length} 条，仅显示前 ${shown.length} 条；输入更精确的关键词继续筛选。</div>` : ""}`;
}
...
function renderSettings() {
  return `
    <div class="card">
      <h2>学习量</h2>
      <div class="field-row">
        <label for="fresh-count">「到期 + 新词」：每轮最多加入的新词数</label>
        <select id="fresh-count" class="text-input" onchange="setSetting('freshPerSession', this.value)">
          ${numberOptions(settings.freshPerSession)}
        </select>
      </div>
      <div class="field-row">
        <label for="due-count">「只做到期」：每轮最多复习的词数</label>
        <select id="due-count" class="text-input" onchange="setSetting('duePerSession', this.value)">
          ${numberOptions(settings.duePerSession)}
        </select>
      </div>
      <div class="field-row">
        <label for="fresh-only-count">「只学新词」：每轮最多学习的词数</label>
        <select id="fresh-only-count" class="text-input" onchange="setSetting('freshOnlyPerSession', this.value)">
          ${numberOptions(settings.freshOnlyPerSession)}
        </select>
      </div>
      <div class="note">三种范围分别设置。练习时会从对应范围里随机抽取不超过设定数量的词；「到期 + 新词」中的到期词会全部出现，新词部分按上面的数量补充。</div>
    </div>
...
function numberOptions(selected) {
  return [5, 10, 15, 20, 25, 30, 40, 50]
    .map((n) => `<option value="${n}" ${selected === n ? "selected" : ""}>${n} 个</option>`)
    .join("");
}
...
async function importProgress(event) {
  ...
  try {
    ...
    if (payload.settings && typeof payload.settings === "object") {
      settings = { ...SETTINGS_DEFAULTS, ...payload.settings };
      saveSettings();
    }
